...
def ShowData(currResults):
    ...
    for i in range(currResults):
        movie_name=movie_data['results'][i]['original_title']
        overview=movie_data['results'][i]['overview']
        image=movie_data['results'][i]['poster_path']

        print('\n\n Name Of Movie ',i+1,': '+movie_name)
        Movie_Name.append(movie_data['results'][i]['original_title'])

        if(overview!=''):
            print(' Overview Of Movie ',i+1,': '+overview,'\n')
            Movie_Overview.append(movie_data['results'][i]['overview'])
        else:
